<template>
	<view class="m-tabbar-box" :style="tabbarBoxStyle" v-if="isShowTabBar">
		<view class="m-tabbar__fill" v-if="fill || native" :class="{'m-tabbar__safe': (safeBottom || native)}" :style="tabbarFillStyle"></view>
		<!-- <view id="m-tabbar" class="m-tabbar" :class="{'fixed': (fixed || native), 'm-tabbar__safe': (safeBottom || native)}"  :style="tabbarStyle"> -->
		<view id="m-tabbar" class="m-tabbar a-h-150 a-bg-white" :class="{'fixed': (fixed || native), 'm-tabbar__safe': (safeBottom || native)}" >
<!-- 			<image class="a-w-750 a-h-200 a-position-absolute a-top-0" src="/static/tabbar/tabbar.png"></image> -->
			<!-- <view class="m-tabbar__border" v-if="borderStyle === 'black' "></view> -->
			<view class="a-w-750 a-h-10"></view>
			<view class="m-tabbar__flex a-position-relative" >
				<view @click="tabChange(index)" v-for="(item, index) in tabbarList" :key="index" class="m-tabbar__item"
					...
					<slot :name="`tabbar_index_${index}`">
						...
						<view class="m-tabbar__label"
							:style="{'color': index === currentIndex ? tabbarConfig.selectedColor : '#DBDBDA' }">
							{{$t(item.text)}}
						</view>
					</slot>
				</view>
		...
		computed: {
			...
			tabbarStyle() {
				...
				return obj2strStyle({
					...
					// 'background':'url(/static/tabbar/tabbar.png);background-size:contain;background-repeat:no-repeat;'
				})
			},
			tabbarItemStyle() {
				const {
