<template>
  <view class="raffle-wheel" :style="{ width: canvasWidth + 'px', height: canvasHeight + 'px'}">
    <view class="raffle-wheel-box" :style="{ width: canvasWidth + 'px', height: canvasHeight + 'px'}">
      <view class="raffle-wheel-show" :style="'transform: rotate(' + rotates + 'deg);transition-duration: ' + duration + 's;'">
        <view class="item-block" v-for="(item, index) in prizeList" :key="index">
		  ...
		  <view class="item-view-block"
		    :style="
		    'transform: rotate(' + (degs * index) + 'deg) skewY(' + skew + 'deg);'
		  ">
          </view>
          <view class="item-view" :style="'transform: translateX(-50%) rotate(' + (degs * index + (degs / 2)) + 'deg);width: ' + textWidth + ';'">
            <text class="a-text-yellow-orange a-font">{{item.cname}}</text>
...
<script>
  export default {
    ...
    methods: {
      ...
      endAction() {
        console.log('本次选中的：' + this.prizeList[this.targetIndex].cname)
      }
    }
  }
</script>
...
<style lang="scss">
  $actionBgUrl: '~static/share/lo-turntable-pointer';
  $raffleBgUrl: '~static/share/lo-turntable';
  .raffle-wheel {
    position: relative;
    margin: 0 auto;
    display: flex;
