 ...
 * 验证是否已登录
 */
export const promptLogin = (title,content,confirmText,cancelText) => {
	console.log(title)
  uni.showModal({
  	title: title,
	content: content,
 ...
 * 获取存储  
*/
export const cache = (key, value = '', time = 180) => {
    if (value) {
        let expire = parseInt(new Date().getTime() / 1000 + time);
        let data = {'value': value, 'expire': expire};
        return uni.setStorage({key: key, data: JSON.stringify(data)});
    } else if (value == null) {
        ...
        if (str) {
            try {
                ...
                if (obj.value && obj.expire > parseInt(new Date().getTime() / 1000)) {
                    return obj.value;
                } else {
                    return '';
                }
 ...
 * 倒计时
 */
export const countdown = (times,type) => {
  if(type=="day"){
