<template>
	<view>
		<u-sticky bgColor="#fff" class="a-position-relative">
			<!-- #ifdef H5 -->
			<view class="a-h-28 a-flex a-align-center a-justify-center a-position-absolute a-right-2 a-bg-red a-px-1 a-rounded-top-2  a-position-absolute" style="top:-1rpx;">
				<text class="a-font-tiny a-text-white">{{$t('user.paymentGift')}}</text>
			</view>
			<!-- #endif -->
			<!-- #ifndef H5 -->
			<view class="a-h-30 a-flex a-align-center a-justify-center a-position-absolute a-right-3 a-top-0 a-bg-red a-px-2 a-rounded-top-2  a-position-absolute">
				<text class="a-font-min a-text-white">{{$t('user.paymentGift')}}</text>
			</view>
			<!-- #endif -->
		<u-tabs class="a-pt-2" style="background:rgb(237 196 118) !important;" :list="tabs" :is-scroll="false" :current="curTab" active-color="#000000" :duration="0.2"
		  @change="onChangeTab" />
		</u-sticky>
		
		<view class="a-flex-column a-justify-center a-px-5 a-w-690 a-mt-5 a-bg-orange-yellow-lighter a-rounded-top-2 a-mx-3 a-h-230 forms-border-bottom">
			<view class="">
		...
		<view v-if="curTab ==0" class="a-mx-3 a-w-690 a-py-6">
			<view class="a-py-3">
			...
			<view class="a-mt-2 a-bg-primary-lighter a-rounded-2">
				<radio-group>
					<label v-for="(item,index) in payment" :key='index' @tap='payItem(index)' class="a-flex a-justify-between a-align-center a-h-100 a-border-light" :class="index !=0?'a-border-top':''">
						...
						<radio color="#2C8FF7" :checked="form.payment_id==item.id"></radio>
					</label>
				</radio-group>
			</view>
		</view>
		
		<view class="a-px-5 a-mt-5">
