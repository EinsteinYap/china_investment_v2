<template>
	...
	<view class="a-flex a-justify-center  a-flex-wrap a-mx-3 a-mb-5 a-px-1 a-py-2 a-w-700" >
		...
		<view v-if="info.task.length" class="a-px-3 a-mb-5">
			...
			<view >
				<view v-if="isScrolled" v-for="(item,index) in info.task" :key="index" @click="$navTo('pages/task/articleTask?id='+item.id)" class="a-flex a-align-center a-bg-primary-op-3 cards-first-last-border-radius  forms-border-bottom animate__animated animate__bounceInRight">
		...
		<view v-if="info.news.length" class="a-px-3 a-mb-5">
			...
			<view class="">
				<view v-if="isScrolled" v-for="(item,index) in info.news" :key="index" @click="$navTo('pages/news/articleNews?id='+item.article_id)" class="a-flex a-align-center a-bg-primary-op-3 cards-first-last-border-radius  forms-border-bottom animate__animated animate__bounceInLeft animate__delay-1s">
