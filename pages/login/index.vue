<template>
  <view v-if="isLoad" class="login login-background a-h-max">
    <MpWeixin v-if="isMpWeixinAuth" @success="onGetUserInfoSuccess" />
    <Main v-else :isParty="isParty" :partyData="partyData" :source="source" :register="register"/>
  </view>
</template>
...
<script>
  ...
  export default {
    ...
    async onLoad(options) {
		if(options.staffId){
			this.source.staffId = options.staffId
		}
		if(options.invitCode){
			this.source.invitCode = options.invitCode
		}
		if(options.register){
			this.source.register=JSON.parse(options.register)
		}else{
			this.source.register=false
		}
